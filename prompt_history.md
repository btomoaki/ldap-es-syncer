...
## [2026-06-21] ステップ23: MITライセンス（LICENSE）の追加

### 概要
プロジェクトの利用・改変・再配布条件を明確にし、ソフトウェアの無保証・免責事項を明文化するため、プロジェクトルートに MIT ライセンスファイル（`LICENSE`）を追加。

### 決定事項
- **ライセンスの選定:** 寛容かつ無保証免責が明記された `MIT License` を採用。
- **著作権者表記:** コミットログの作成者名に基づき `btomoaki` を著作権者として指定し、年表記を `2026` とする。
...
## [2026-06-21] ステップ25: READMEのローカルファイルリンクの修正

### 概要
`README.md` 内に記述されている Helm 関連のリンクがローカルファイルパス（`file:///...`）を指しているため、GitHub上で正しく遷移できるようにリポジトリのコンテキストパスを用いた指定（`/btomoaki/ldap-es-syncer/...`）に変更する。

### 決定事項
- **リンク参照の修正:** ローカル絶対パスで指定されていた以下の2箇所のリンクを、GitHub上のコンテキストパスまたは相対パス（リファレンス形式）に変更。
  - Native ユーザー同期の制約事項へのリンク
  - デプロイマニュアル（Helm README）へのリンク
